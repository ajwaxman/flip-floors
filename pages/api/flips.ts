...
const fetchFlipFloor = async (name) => {
    let url = "https://indexer-v31-mainnet.up.railway.app/tokens?collection=flipmap&sortBy=floorSellValue&sortDirection=asc&offset=0&limit=6&attributes%5BComposition%5D="
...
const fetchLevelFloor = async (id) => {
    let url = 'https://indexer-v31-mainnet.up.railway.app/tokens?collection=terraforms&sortBy=floorSellValue&sortDirection=asc&offset=0&limit=1&attributes%5BLevel%5D='
    url += id

    const res = await fetch(url)
    const json = await res.json()
    return json["tokens"][0]["floorSellValue"]
}
